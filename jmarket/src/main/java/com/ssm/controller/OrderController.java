...
//        System.out.println(notices.toString());
        notices.setOrderid(order.getId());
//        System.out.println(order.getId());
        //用户购买成功后给商家发送购买信息
        notices.setUserstatus("用户未收货");
    ...
    @RequestMapping(value = "/updateOrderUserStatus",produces={"text/html;charset=UTF-8;"})
    @ResponseBody
    public int updateOrderUserStatus(int orderId, String status){

        Notices notices=new Notices();
        int q1=notices.getOrderid();
    ...
    //商家发货
    @RequestMapping(value = "/fahuo", produces = {"text/html;charset=UTF-8;"})
    @ResponseBody
    public String fahuo(
            @RequestBody Notices notices
    ) {
//        System.out.println(notices);
        Order order=orderService.findOrderByOrderid(notices.getOrderid());
//        System.out.println(order);
        if(order.getOrderstatus().equals("未发货")){
            order.setOrderstatus("已发货");
        }else{
            return "已经发货了!";
        }
//        System.out.println(order);
        int i = orderService.fahuo(order);
    ...
    //用户收货
    @RequestMapping(value = "/shouhuo" , produces = {"text/html;charset=UTF-8;"})
    @ResponseBody
    public String shouhuo(
            @RequestBody Order order
    ){
        System.out.println(order);
        Notices notices=orderService.findNoticeByOrderid(order.getId());
        if(!order.getOrderstatus().equals("未发货")){
            notices.setUserstatus("用户已收货");
        } else {
            return "商家还没有发货!";
        }

        int i = orderService.shouhuo(notices);
        if(i == 0){
            return "交易失败!";
        }
        return "交易成功!";
